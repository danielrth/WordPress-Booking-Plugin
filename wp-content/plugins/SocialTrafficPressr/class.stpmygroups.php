...
class stpMyGroupsTable extends WP_List_Table {
...
function process_bulk_action() {
 ...
 function prepare_items($campaign) {
	...
	$table_name = $wpdb->prefix ."stpMyGroups"; 
	
	   $orderby = $_REQUEST['orderby'];
	   $order = $_REQUEST['order'];
	   
	   if (empty($orderby)){
		$query = "SELECT * FROM $table_name WHERE (node_id != '2147483647' ) ORDER BY  id desc ";    
	   }
	   
		   
	     else if($orderby == "title"){
		$query = "SELECT * FROM $table_name  WHERE (node_id != '2147483647' ) ORDER BY title $order ";
	   }
	   
	  else{
		$query = "SELECT * FROM $table_name  WHERE  (node_id != '2147483647') ORDER BY CAST(`$orderby` AS SIGNED) $order  ";    	
	    }
	/*    else {
